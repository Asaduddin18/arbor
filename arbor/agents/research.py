"""Research agent for Arbor v2.

Executes information gathering and synthesis tasks.
...
_RESEARCH_TASK_INSTRUCTIONS = """
You are executing a research task. Produce a Markdown document with:
...
Under ## Findings: present your findings clearly and factually.
Under ## Sources: cite every source used (URL, document name, or "internal context from memory/...").
Do not claim certainty where you are not certain. Use phrases like "likely", "appears to be",
"according to [source]" for claims you cannot verify from provided context.
"""
...
class ResearchAgent(BaseAgent):
    ...
    async def execute_task(
        ...
        user_message = f"Execute this research task:\n\n**Task ID:** {task_id}\n**Goal:** {goal}"
        if retry_feedback:
            user_message += f"\n\n{retry_feedback}"
